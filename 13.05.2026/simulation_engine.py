...
class SimulationEngine:
    ...
    def step(self) -> None:
        ...
        # Update health states for all citizens
        for citizen in self.citizens.values():
    ...
    def handle_deaths(self) -> None:
        ...
        for citizen_id, citizen in self.citizens.items():
            ...
            # --- 1. Akumulacja hazardu + onset --------------------------------
            # Zapewnij że H_cum istnieje dla wszystkich chorób (lazy init dla
            # noworodków stworzonych z pustym dict)
            new_onset = False
            for disease in diseases_list:
                ...
                # Onset — Poisson approx dla rzadkich zdarzeń per krok
            ...
            # Burden chorobowy (jak dotychczas, DW-based)
            disease_multiplier = 1.0 + 0.04 * citizen.disability_score

            # Cox-style multiplier z akumulowanego hazardu aktywnych chorób
    ...
    def handle_births(self) -> None:
        ...
        for citizen in self.citizens.values():
            if citizen.alive and citizen.sex == "female":
                ...
                # Reduce fertility based on disease burden (small adjustment — floor at 0.7)
