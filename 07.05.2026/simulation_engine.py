...
class SimulationEngine:
    ...
    def handle_deaths(self) -> None:
        ...
        for citizen_id, citizen in self.citizens.items():
            ...
            # Modify by disease burden (small adjustment — base tables already include disease effects)
            disease_multiplier = 1.0 + (0.02 * citizen.num_conditions())
            disease_multiplier += 0.04 * citizen.disability_score

            # Modify by risk factors (reduced to avoid double-counting with calibrated base tables)
            risk_multiplier = 1.0
