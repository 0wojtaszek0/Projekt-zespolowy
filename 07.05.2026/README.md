...
7. [Tabele demograficzne](#7-tabele-demograficzne)
...
```python
engine.fertility_rate = 1.0          # mnożnik płodności (FM)
engine.mortality_multiplier = 1.0    # mnożnik śmiertelności (MM)
engine.household_split_probability = 0.001  # 0.1%/miesiąc
```

**Ważne**: `fertility_rate` w kodzie to tak naprawdę `fertility_multiplier` — mnożnik całej tabeli ASFR. Przy FM=1.0 dostajemy polskie dane GUS 2021 (TFR≈1.26). Przy FM=2.0 każda stopa płodności dla danego wieku jest podwojona.
