...
✅ **Choroby modelowane**: 3 główne choroby (CVD, Lung Cancer, Hypercholesterolemia)
...
✅ **Interaktywne wizualizacje**: Plotly z suwakami rocznymi (piramidy wieku, trendy populacji)
✅ **Optymalizacja parametrów**: GridSearch z ilustracją heatmap (2D) i powierzchniami 3D

### Nowe funkcjonalności

- **Modelowanie stanów zdrowia**: Wprowadzenie stanów zdrowia ("healthy", "exposed", "infected", "severe", "recovered") oraz ich przejść w oparciu o prawdopodobieństwa.
- **Dynamiczna aktualizacja zdrowia**: Obywatele zmieniają swoje stany zdrowia w trakcie symulacji w zależności od zdefiniowanych prawdopodobieństw przejść.
- **Integracja z symulacją**: Logika przejść między stanami zdrowia została zintegrowana z głównym silnikiem symulacji.
...
├── disease_model.py                 # Klasa DiseaseModel (3 choroby)
├── simulation_engine.py             # Klasa SimulationEngine
├── visualization.py                 # Klasa SimulationVisualizer
├── heatmap_gridsearch.py            # GridSearch z wizualizacją heatmap
├── population_data.xlsx             # Dane wejściowe (opcjonalnie)
├── piramida_wieku_rok_50.html       # Piramida wieku roku 50
├── piramida_wieku_animowana.html    # Piramida wieku animowana
...
├── heatmap_gridsearch.html          # Mapy ciepła GridSearch (2D)
├── heatmap_3d_*.html                # Wykresy 3D parametrów
...
**Stale**:
- `DEFAULT_DISEASES` - 3 główne choroby:
  - **CVD (Cardiovascular Disease)**: 35.0% prevalence, 0.25 disability weight
  - **Lung Cancer**: 4.5% prevalence, 0.55 disability weight
  - **Hypercholesterolemia**: 25.0% prevalence, 0.08 disability weight
...
- 3 kolumny chorób (0/1):
  - **CVD** (35.0% prevalence)
  - **Lung Cancer** (4.5% prevalence)
  - **Hypercholesterolemia** (25.0% prevalence)

W przypadku braku pliku Excel, system automatycznie generuje populację syntetyczną.
...
Program generuje **9 interaktywnych HTML plików**:

### 📊 Part 1: Piramidy Wieku
- **piramida_wieku_rok_50.html** - Piramida wieku roku 50
- **piramida_wieku_animowana.html** - Animowana piramida z suwakiem (0-50 lat)

### 📈 Part 2: Trendy Populacyjne
- **population_trends.html** - Całkowita populacja, multimorbidność, disability score
- **households_trends.html** - Liczba gospodarstw, średnia wielkość
- **gender_distribution.html** - Rozkład płci (M/F) w czasie

### 🔥 Part 3: Mapy Ciepła GridSearch (4D)
- **heatmap_gridsearch.html** - 2D heatmapy relacji parametrów
- **heatmap_3d_birth_rate_vs_fertility_multiplier.html** - 3D powierzchnia
- **heatmap_3d_birth_rate_vs_mortality_rate.html** - 3D powierzchnia
- **heatmap_3d_fertility_multiplier_vs_mortality_multiplier.html** - 3D powierzchnia

## Optymalizacja GridSearch - Wyniki

**Ostatnia optymalizacja (2026-04-08)** zidentyfikowała optymalne parametry dla populacji docelowej 30,000:

### GridSearch 4D (bez migracji)

Zmniejszono wymiarowość z 5D na 4D poprzez usunięcie zmiennej `migration_rate`, co przyspieszyło poszukiwanie optymalnych parametrów.

```
Optymalne parametry:
  • birth_rate: 0.058
  • fertility_multiplier: 2.3
  • mortality_multiplier: 0.5
  • mortality_rate: 0.0005

Ocena (4D): 25,872,409.22
(wcześniej 5D z migracją: 32,862,170.64)
```

Parametry te są automatycznie wczytywane w `main.py` z wyników GridSearch. Można też je modyfikować ręcznie:
...
```python
engine.fertility_rate = 2.3                    # Optymalnie: 2.3
engine.mortality_multiplier = 0.5              # Optymalnie: 0.5
